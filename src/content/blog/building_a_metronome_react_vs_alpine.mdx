...
The goal of this article is to compare React and Alpine by building a simple [metronome](https://en.wikipedia.org/wiki/Metronome).
We're going to take a very simple approach, so keep in mind that the metronome we're building is not going to be the
best. If you want to implement time-based or sound-related behavior, a better approach would be to make use of the
...
time, declare a `timer` variable to store the interval ID, and create an [HTMLAudioElement](https://developer.mozilla.org/en-US/docs/Web/API/HTMLAudioElement)
